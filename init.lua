...
-- path_to_uploads = modpath .. DIR_DELIM .. "uploads" .. DIR_DELIM
...
display_max_size = 50
display_min_size = 1
display_max_textures = 60

displays = {}
nextDisplayIndex = 0
insecure_environment = nil

max_download_length = 10097152 --2MB Increase this to allow bigger images
...
function  DisplayEntity:update_size()
   ...
   -- # if yaw == 0 then
        self.object:set_properties({
            ...
            collisionbox = {-half_x, -half_y, 0, half_x, half_y, 0}
    })
--#   else a
 -- #      self.object:set_properties({
  -- #         visual_size = {x = size_x, y = size_y},
...
function DisplayEntity:show_formspec(clicker)
    ...
    "field[3,3;1,.5;R_CustomYaw;;".. self.object:get_yaw() .."]"..
...
function handle_display_form(player, formname, fields)
    ...
    if fields.Rotate then
        ...
        display.object:set_yaw((3.1415926535/tonumber(fields.R_CustomYaw)))
        display:update_size()
    end
